...
public class ForgetpasswordActivity extends BaseActivity<ForgetView, ForgetPeresenter> implements ForgetView {
    ...
    Button forget;
    private ForgetBean forgetBean1 = new ForgetBean();
    private String msg;
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.forget:
                Gson gson = new Gson();
                String s = gson.toJson(new ForgetJson(phone.getText().toString()));
                ToastUtil.showLong(s);
                mPresenter.getForgetModel(s, phone.getText().toString());
                break;
            case R.id.btn_next:
